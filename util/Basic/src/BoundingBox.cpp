...
bool BoundingBox::checkIntersect(const Line& _l){
    ...
    if((_l.m_StartPoint[0] >= m_MinXYZ[0] && _l.m_StartPoint[0] <= m_MaxXYZ[0])&&
        ...
        float t1 = (m_MaxXYZ[1] - _l.m_StartPoint[2])*invy;
        float t2 = (m_MinXYZ[1] - _l.m_StartPoint[2])*invy;
        float t3 = (m_MaxXYZ[0] - _l.m_StartPoint[0])*invx;
        float t4 = (m_MinXYZ[0] - _l.m_StartPoint[0])*invx;
        float t5 = (m_MaxXYZ[2] - _l.m_StartPoint[2])*invz;
        float t6 = (m_MinXYZ[2] - _l.m_StartPoint[2])*invz;
        ...
        i *= BoundingBox::isNotin2DPlane(m_MaxXYZ[1],m_MinXYZ[1],m_MaxXYZ[2],m_MinXYZ[2],p1[1],p1[2]);
        i *= BoundingBox::isNotin2DPlane(m_MaxXYZ[1],m_MinXYZ[1],m_MaxXYZ[2],m_MinXYZ[2],p2[1],p2[2]);
        i *= BoundingBox::isNotin2DPlane(m_MaxXYZ[0],m_MinXYZ[0],m_MaxXYZ[2],m_MinXYZ[2],p3[1],p3[2]);
        i *= BoundingBox::isNotin2DPlane(m_MaxXYZ[0],m_MinXYZ[0],m_MaxXYZ[2],m_MinXYZ[2],p4[1],p4[2]);
        i *= BoundingBox::isNotin2DPlane(m_MaxXYZ[0],m_MinXYZ[0],m_MaxXYZ[1],m_MinXYZ[1],p5[1],p5[2]);
        i *= BoundingBox::isNotin2DPlane(m_MaxXYZ[0],m_MinXYZ[0],m_MaxXYZ[1],m_MinXYZ[1],p6[1],p6[2]);
        if(i==1){
            return false;
        }
        else{
